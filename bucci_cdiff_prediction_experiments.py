...
def prediction_experiment(Y, U, T):
    ...
    ax.set_title("$\it{C. diff}$ Dataset")


    # df = pd.DataFrame(np.array([baseline_err_cv, en_err_cv, glv_err_cv, linear_err_cv, rel_abun_err_cv]).T,
    #                   columns=["baseline",
    #                            "cLV",
    #                            "gLV",
    #                            "alr",
    ...
    plt.savefig("plots/bucci-cdiff_prediction-comparison.pdf")
    ...
    for idx, en_glv_linear_rel in enumerate(zip(en_err_stratified_cv, glv_err_stratified_cv, linear_err_stratified_cv, rel_abun_err_stratified_cv)):
        ...
        plt.savefig("plots/bucci-cdiff_prediction_comparison-test-{}.pdf".format(idx))
